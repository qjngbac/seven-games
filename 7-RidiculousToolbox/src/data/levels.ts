...
export const LEVELS: Level[] = [
  ...
  // ---------- L1 服务器过热 ----------
  {
    ...
    recipes: [
      ...
      { recipeId: 'l1_intake', kind: 'use', inputs: [{ tag: 'cable' }], target: { id: 'server' }, outputs: [{ setScene: { targetId: 'server', state: { intakeBlocked: false } } }], feedback: '你用网线捅了捅进风口，灰尘飞扬。', priority: 5, category: 'neutral' },
      ...
      { recipeId: 'l1_f_cable_tape', kind: 'combine', inputs: [{ tag: 'cable' }, { tag: 'adhesive' }], feedback: '你把自己缠成了木乃伊，胶带很满意。', galleryNote: '自助捆绑体验。', priority: 5, category: 'failure' },
  ...
  // ---------- L5 火灾警报误报 ----------
  {
    ...
    recipes: [
      ...
      { recipeId: 'l5_absurd', kind: 'use', inputs: [{ tag: 'cold' }], target: { id: 'alarm' }, outputs: [{ setScene: { targetId: 'alarm', state: { silenced: true, ringing: false } }, setFlag: { key: 'solvedTier', value: 'absurd' } }], scores: ABS, feedback: '冰淇淋贴上去，警报器被冰得一哆嗦不响了。离谱降温静音。', priority: 10, category: 'solution' },
      ...
      { recipeId: 'l5_f_water_soap', kind: 'combine', inputs: [{ tag: 'liquid' }, { tag: 'clean' }], feedback: '肥皂水，能洗但灭不了警报。', galleryNote: '肥皂水非灭火。', priority: 5, category: 'failure' },
  ...
  // ---------- L8 堵住的马桶 ----------
  {
    ...
    recipes: [
      { recipeId: 'l8_pro', kind: 'use', inputs: [{ tag: 'container' }], target: { id: 'toilet' }, outputs: [{ setScene: { targetId: 'toilet', state: { flushed: true, clogged: false } }, setFlag: { key: 'solvedTier', value: 'professional' } }], scores: PRO, feedback: '瓶子倒扣当皮搋子，一压一拔，哗啦通了。专业疏通。', priority: 10, category: 'solution' },
      ...
      { recipeId: 'l8_temp', kind: 'use', inputs: [{ tag: 'container', requireState: { full: true } }], target: { id: 'toilet' }, outputs: [{ setScene: { targetId: 'toilet', state: { flushed: true, clogged: false } }, setFlag: { key: 'solvedTier', value: 'temporary' } }], scores: TMP, feedback: '整桶水猛冲下去，水压冲开了堵塞。临时大水漫灌。', priority: 10, category: 'solution' },
  ...
  // ---------- L16 下水道反味 ----------
  {
    ...
    recipes: [
      ...
      { recipeId: 'l16_absurd', kind: 'use', inputs: [{ tag: 'cold' }], target: { id: 'drain' }, outputs: [{ setScene: { targetId: 'drain', state: { sealed: true, smelly: false } }, setFlag: { key: 'solvedTier', value: 'absurd' } }], scores: ABS, feedback: '冰塞住漏口，冻住气味。离谱但凉快。', priority: 10, category: 'solution' },
