...
npm test           # 运行全部单元测试（含 20 关批量唯一匹配 + 可达性验证）
```
...
20 关（5 章，每章 4 关，共 239 条配方 / 60 条解法），每关含专业 / 临时 / 离谱三解 + 独特失败反馈：
...
- **第五章**：17. 卡住的拉链 · 18. 生锈的螺丝 · 19. 打不开的果酱罐 · 20. 停电的夜晚

（选择关卡页面按章分组，每章标题为「第N章」；下一关按章节顺序推进。）

## 验证结果

- `npm test`：**30/30 通过**——规则内核（组合匹配 / 条件 / 优先级 / 工具不消耗 / 通用失败）、评分、撤销重做、校验器（优先级冲突 + 引用完整性 + **工具消耗语义一致性**）、章节分组（5 章 × 4 关），以及 **20 关全部唯一匹配且至少一解可达**（BFS 可达性搜索，附解法路径）。
- **同优先级歧义由「更具体的规则」消解**：如橡胶块同时带 `rubber`/`adhesive` 标签时，命中「橡胶堵漏（专业）」而不是「胶带缠缝（离谱）」；不再依赖配方声明顺序。
...
src/
  ...
  data/         内容（纯数据）
    ...
    levels.ts      20 关：场景热点 / 初始物品 / 配方 / 解法谓词 / 简报
    feedback.ts    通用失败规则表
    content.spec.ts 20 关批量校验测试
  puzzle/       开发期工具
    validator.ts   引用完整性 + 优先级冲突 + 工具消耗一致性 + 至少一解
