...
src/
  ...
  data/
    rule-packs/*.json      6 个数据驱动规则包（共 34 条规则）
    packs.ts               编译 + 校验所有规则包
```
...
| 逻辑组合 | 4 | 7 | 多机制前提组合（与/或/非） |
| 极限混战 | 5 | 10 | 上述规则大乱斗 |

所有规则用结构化字段表达（谓词 + 动作 + 优先级 + 展示文本），**文本与执行同源**，加载时做冲突检测，绝不进入有歧义的对局。
加载期还会做**揭示覆盖校验**：保证在普通/每日模式的 30 轮内，每条规则都一定能被揭示（不会出现"整局永不生效"的规则）；
以及**双重反转校验**：两条 `INVERT_BASE` 规则若可能同时命中（会互相抵消、玩家无法预判），直接拒绝该规则包。
...
- **揭示覆盖**：6 个规则包在普通模式 30 轮内均能把整包规则全部揭示（逐包断言）。
- **编译期拦截**：双重反转（两条 `INVERT_BASE` 谓词重叠）被拒；不同优先级的同谓词覆盖属正常机制，不算冲突。
